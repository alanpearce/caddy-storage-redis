...
func (rs *RedisStorage) Repair(ctx context.Context, dir string) error {
	...
	if dir == "" {
		...
		for {
			...
			for _, key := range keys {
				...
				if err != nil {
					rs.logger.Info("Unable to load storage data", zap.String("key", trimmedKey))
					continue
				}
	...
	for _, k := range keys {
		...
		if !rs.Exists(ctx, fullPathKey) {
			...
			rs.logger.Info("Removed non-existent record from directory",
				zap.String("record", k),
				zap.String("directory", currKey),
			)
			continue
		}
...
func (rs RedisStorage) storeDirectoryRecord(ctx context.Context, key string, score float64, repair, baseIsDir bool) error {
	...
	if success > 0 || repair {
		if success > 0 && repair {
			rs.logger.Info("Repaired index", zap.String("record", base), zap.String("directory", dir))
		}
